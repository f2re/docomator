...
Продуктовый код основного сценария доведён до code-complete состояния. `0.6.3` закрывает оставшийся пользовательский разрыв мастера шаблона: форматированное DOCX/XLSX представление доступно локально, сохраняет CSP без `unsafe-inline`, не исполняет содержимое Office и не меняет детерминированный renderer.
