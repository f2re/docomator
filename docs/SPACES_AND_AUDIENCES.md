...
1. **Клиент приложения** — браузер или иной клиент, допущенный внешним корпоративным периметром. Оформлятор не создаёт ему локальную учётную запись, роль или членство и показывает все разделы.
...
Это разделение обязательно: клиент приложения не обязан быть участником формируемого документа, а человек в документе не становится учётной записью Оформлятор.
...
> [!NOTE]
> Выбор, проверка, снимок и target plan реализованы. Одна строка таблицы DOCX уже физически повторяется в сводном документе по неизменяемому порядку снимка; диапазоны XLSX, списки и более сложные структурные виды остаются следующими этапами.
