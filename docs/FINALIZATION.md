...
Любые evidence для `0.1.0` остаются историей и не закрывают gate `0.3.2`.
...
2. не менять номер `0.3.2` только ради смены статуса; stable должен быть новым commit с тем же version и новой release identity, если продуктовый состав не изменился;
