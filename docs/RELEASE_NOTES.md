...
- Первая версия collection-repeat намеренно ограничена одним уровнем вложенности и одной зоной на шаблон. Nested entity-collection repeat для XLSX в `0.7.0` не заявляется; существующий XLSX repeat по `audience.members` остаётся без изменения.
- Версия поднята штатно до `0.7.0`. После успешного merge в `main` и зелёного post-merge CI GitHub Release `v0.7.0` публикуется автоматическим release workflow; тот же контракт применяется к каждой следующей новой версии.
