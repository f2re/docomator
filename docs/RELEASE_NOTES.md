...
## 2026-09-09 — канонический контроллер CSV/XLSX-импорта (`0.7.4`)

- Выбор файла, drag&drop, space-scoped память сопоставлений и адресная работа со структурированными ошибками сведены в `bulk-data-import-controller.js`; поздний runtime-overlay `space-isolation-ui.js` удалён.
- Контроллер использует машинные `rowNumber`, `column`, `rawValue` и `suggestedAction` для подсветки проблемной строки/ячейки и конкретного действия; regexp-разбор локализованного сообщения не добавлен.
- Ошибка проверки сохраняет выбранный файл, сопоставления и preview; корректные строки могут быть импортированы отдельно, а проблемные остаются явно отмеченными.
- Regression checks запрещают возвращать `MutationObserver`, `setTimeout`, синтетический `.click()` и отдельный post-hoc import overlay; память сопоставлений привязана к текущему разделу данных.
- API/storage/schema/migrations/renderer/security boundary не менялись. #153 остаётся открыт для следующего среза карточки данных и toolbar; версия продукта синхронизирована как `0.7.4 / candidate / pilot`.

## 2026-09-07 — импорт начинает проверку без синтетического клика (`0.7.3`)

- Выбор CSV/XLSX и drag&drop теперь передаются отдельному контроллеру импорта, который запускает чтение сразу, без фиксированной задержки 900 мс и синтетического `.click()`.
- Базовый модуль импорта остаётся источником preview, сопоставления колонок, структурированных ошибок и сохранения состояния; ручная кнопка проверки сохранена как безопасный fallback.
- Статические regression checks запрещают возвращать `setTimeout`, синтетический click и `MutationObserver` в путь старта импорта.
- Существующие правила определения типа, enum и inline-repair пока сохранены без изменения; перенос оставшихся import monkey-patch в канонический controller остаётся следующим срезом #153.
- Storage/API/renderer/security boundary и migrations не изменялись. Release identity, workspace packages, lockfile и runtime defaults синхронизированы с `0.7.3`.

## 2026-09-05 — канонические Home, topbar и «Управление» (`0.7.2`)

- Home, системный status control, переключатель раздела данных, индикатор этапа и экран «Управление» теперь присутствуют в исходной разметке и не создаются поздним JS-композитором.
- `interface-hierarchy.js` сокращён до синхронизации уже существующих элементов с фактическим состоянием: он больше не строит topbar/Home/Management, не переименовывает навигацию, не перемещает диагностику и не использует `setTimeout` для достижения финального DOM.
- Диагностика готовности создаётся сразу в `#managementReadinessMount` и обновляется автоматически только при открытом экране «Управление»; ежедневный Home больше не превращается в операторскую панель.
- Home закрепляет один путь «Данные → Шаблон → Выпуск → Результат» и показывает backend-derived готовность этапов без второго конкурирующего мастера.
- CI regression проверяет, что канонические shell/Home/Management элементы существуют до запуска feature JS и что post-hoc composer-функции не возвращаются.
- GitHub Actions остаётся упрощённым read-only `Essential checks`; browser/offline/target/release acceptance не возвращены в каждый PR.
- Storage/API/renderer/security boundary и migrations не изменялись. Release identity и workspace metadata синхронизированы с `0.7.2`.

## 2026-09-05 — каноническая навигация без поздней перестройки DOM (`0.7.1`)

- Первичная desktop-навигация теперь создаётся сразу в каноническом виде: «Главная · Сотрудники · Шаблоны · Создать документы · Результаты · Расписания · Управление», без временного пункта «Объекты».
- `navigation-contract.js` стал декларативным registry и больше не удаляет и не переставляет DOM, не использует `MutationObserver` и не создаёт secondary links постфактум.
- `field-groups-ui.js` больше не владеет загрузкой навигационного контракта; shell подключает его перед основным приложением.
- CI фиксирует исходный desktop/mobile contract и запрещает возвращать DOM rewrite в navigation contract.
- Изменение не затрагивает storage/API/renderer/security boundary; release identity, workspace packages, lockfile и runtime defaults синхронизированы с `0.7.1`.
