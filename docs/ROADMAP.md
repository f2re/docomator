...
- единый runtime-контракт нормализует desktop sidebar и mobile bottom navigation после подключения всех модулей: ежедневные задачи остаются на верхнем уровне, редкие инструменты доступны через «Управление»/«Ещё»;
- «Управление» использует спокойный одноколоночный реестр, а дополнительный раздел сохраняет видимый текущий пункт навигации;
...
10. два новых пользователя, ручная accessibility/P5-приёмка, включая визуальную разметку DOCX без знания OOXML и понятность новой навигационной иерархии;
