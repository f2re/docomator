...
2. сквозной acceptance `пространство → сотрудники/группа → пользовательские поля → шаблон → проверка и формирование → результат` и `DOCX/XLSX → авторазбор → визуальная коррекция → batch → CSV`, включая изоляцию второго пространства;
