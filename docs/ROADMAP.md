...
Поддержка `0.7.0` намеренно ограничена одной entity-collection repeat-зоной и одним уровнем вложенности в персональном DOCX. Nested entity-collection repeat для XLSX не заявляется; существующий `audience.members` repeat DOCX/XLSX остаётся без изменения.
