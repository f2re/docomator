...
1. полный зелёный repository/Chromium/real-stack/offline CI exact `0.6.5`;
2. сквозной acceptance `пространство → сотрудники/группа → пользовательские поля → шаблон → заполненный документ`, включая изоляцию второго пространства;
