...
Каталог [`examples`](../examples/README.md) проходит детерминированную проверку production intake/structure, compile/render/reverse-read и включается в offline bundle с отдельным SHA-256 manifest. Это безопасный синтетический набор для первого сценария, но не доказательство совместимости конкретной версии LibreOffice/Office. Настоящие Office-сохранённые документы заказчика и их происхождение добавляются только после очистки от персональных данных и проверки права распространения.
