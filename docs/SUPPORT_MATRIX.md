...
Акты предыдущих версий остаются историческими и **не закрывают** acceptance `0.7.3`. Любое пользовательски заметное изменение shell/flow требует нового UX evidence exact `0.7.3`; target/Office/recovery evidence также должно быть связано с exact release binding.
