...
- реальный сценарий `пространство → сотрудники/группа → пользовательские поля → коллекции → шаблон → заполненный документ`, включая два владельца с разным количеством строк, выбор длинного списка без обязательного поиска и изоляцию другого пространства;
- проверку пустой коллекции и сохранения порядка/reorder в персональном DOCX;
...
Collection-repeat в `0.7.0` фактически заявлен для персональных DOCX. Существующий повтор `audience.members` продолжает поддерживать DOCX/XLSX. Nested entity-collection repeat для XLSX не считается поддержанным, пока не появятся отдельные fixture, renderer/reverse-read regression и target acceptance.
...
Акты `0.1.x—0.6.5` остаются историческими и **не закрывают** acceptance `0.7.0`. Добавление entity collections и нового DOCX collection-repeat требует нового evidence exact `0.7.0`, даже при неизменной security boundary ADR-0011.
