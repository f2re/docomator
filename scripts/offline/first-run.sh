...
Откройте интерфейс; при первом запуске создайте общий пароль прямо на странице:
  $UI_URL/
...
Рабочий порядок:
  1. 🔒 Откройте интерфейс и при первом запуске создайте общий пароль на странице.
  2. 🔑 Последующие входы выполняются по этому общему паролю; локальный set-password.sh остаётся запасным способом.
