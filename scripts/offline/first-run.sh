...
Рабочий порядок:
  1. 🧑‍🤝‍🧑 Создайте или выберите пространство.
     Пространство изолирует участников, группы, шаблоны, задания и результаты.

  2. 🧱 Добавьте нужные типы и свойства данных.
     Например: ФИО, должность, подразделение, дата, номер документа и реквизиты.

  3. 👥 Добавьте участников и при необходимости сохраните группы.
     Для разового выпуска можно отметить произвольных людей без создания группы.

  4. 🛡️ Загрузите DOCX или XLSX, проверьте безопасность и сохраните исходник.
     Исходный файл сохраняется неизменяемо по SHA-256.

  5. 🧭 Постройте структуру и сохраните изменяемые места как поля.
     Для каждого поля задаются понятное название, технический ключ, тип и обязательность.

  6. 🧩 Проверьте полный набор полей одной копией.
     Система компилирует все привязки, заполняет значения и считывает их обратно.

  7. 👁️ Создайте PDF, просмотрите результат и активируйте версию.
     Активная версия становится доступна для рабочих заданий пространства.

  8. 📄 Откройте «Сформировать документы для участников».
     Выберите активный шаблон, всех участников, группу или отмеченных вручную.

  9. 🔀 Выберите режим:
     • «Отдельный документ на каждого» — персональный DOCX/XLSX для каждого участника;
     • «Один сводный документ» — одна таблица со строкой для каждого участника.

  10. 🔎 Проверьте обязательные данные до запуска.
      Недостающие значения можно заполнить на том же экране. После сохранения
      система повторно проверит тот же зафиксированный состав.

  11. ▶️ Запустите формирование и скачайте файл или ZIP-комплект.
      Ошибка одного участника не блокирует готовые индивидуальные документы.
      Неуспешные строки можно повторить отдельным заданием.
...
Что уже работает:
  ...
  ✅ один сводный DOCX/XLSX для выбранного состава;
  ✅ отдельный DOCX/XLSX на каждого участника и ZIP-комплект;
  ✅ предварительная проверка обязательных данных;
  ✅ быстрое заполнение пропусков прямо перед выпуском;
