...
PASSWORD_HASH="$(read_value DOCOMATOR_ACCESS_PASSWORD_HASH)"
if [[ -z "$PASSWORD_HASH" ]]; then
  printf '🔒 Оформлятор пока закрыт: общий пароль не настроен.\n'
  printf '   Задайте его локально на сервере:\n'
  printf '   sudo bash /opt/docomator/current/app/scripts/offline/set-password.sh\n\n'
else
  printf '✅ Общий пароль Оформлятор настроен.\n\n'
fi
...
После настройки общего пароля откройте интерфейс:
  $UI_URL/
...
Модель работы:
  • вход выполняется по одному общему паролю Оформлятор;
  • персональных учётных записей, ролей и отдельных прав внутри приложения нет;
  • после входа все допущенные пользователи работают с общими данными и документами;
  • разделы нужны для организации участников, групп, шаблонов и расписаний, а не для разграничения прав;
  • готовые ручные и автоматические результаты попадают в общий раздел «Результаты».

Рабочий порядок:
  1. 🔒 Если пароль ещё не задан: sudo bash /opt/docomator/current/app/scripts/offline/set-password.sh
  2. 🔑 Откройте интерфейс и войдите по общему паролю.
  3. 📥 Импортируйте участников из CSV/XLSX или добавьте их вручную.
  4. 🧱 Проверьте поля и группы сотрудников.
  5. 📤 При необходимости выгрузите данные текущего раздела в CSV или XLSX.
  6. 🛡️ Загрузите и проверьте DOCX/XLSX.
  7. 🧭 Отметьте поля и выполните многополевую проверку.
  8. 👁️ Создайте PDF и активируйте версию шаблона.
  9. 📄 Выпустите один сводный документ или файлы на каждого участника.
  10. 🔎 Исправьте обязательные пропуски до запуска.
  11. 🗓️ При необходимости создайте расписание и настройте почтовую или сетевую доставку.
  12. 📥 Откройте «Результаты»: новый документ остаётся там до скачивания или удаления.
  13. 💾 Убедитесь, что таймер резервирования активен и создана первая копия.
  14. 🗑️ Удаляйте результаты явно и периодически очищайте объекты без ссылок.

Что уже работает:
  ✅ общий пароль входа без пользователей и ролей;
  ✅ экспорт пользовательских данных текущего раздела в CSV/XLSX;
...
Эксплуатационная приёмка всё ещё обязательна:
  ⏳ чистая Debian/Astra Linux;
  ⏳ реальные Office-шаблоны;
  ⏳ update/rollback и восстановление на отдельном стенде;
  ⏳ пользовательская приёмка без устной инструкции.
EOF
