...
test("first-run показывает код доступа и только действия пользователя, а не release backlog", () => {
  ...
  assert.match(output, /Логин не нужен/u);
  assert.match(output, /reset-access-code\.sh/u);
