...
[[ "$VERSION" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || die "Версия содержит запрещённые символы: $VERSION"
[[ -n "$PREVIEW_PROFILE" ]] || die "Укажите --with-preview или --without-preview; preview-профиль не выбирается неявно."
[[ -n "$UX_ACCEPTANCE_PROFILE" ]] || die "Укажите --with-ux-acceptance или --without-ux-acceptance; профиль UX-приёмки не выбирается неявно."
...
[[ "$TARGET_PROFILE" == "generic" || "$TARGET_PROFILE" == "debian" || "$TARGET_PROFILE" == "astra" ]] || die "Неподдерживаемый target-profile: $TARGET_PROFILE"
if [[ "$TARGET_PROFILE" == "generic" && -n "$OS_PACKAGES_DIR" ]]; then die "Профиль generic не должен содержать пакеты конкретной ОС."; fi
if [[ "$TARGET_PROFILE" != "generic" && -z "$OS_PACKAGES_DIR" ]]; then die "Профиль $TARGET_PROFILE требует --os-packages-dir с полным замыканием зависимостей."; fi
...
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
  ...
  [[ "$UX_CHROMIUM_PACKAGE" =~ ^[a-z0-9][a-z0-9+.-]*$ ]] || die "Некорректное имя пакета Chromium: $UX_CHROMIUM_PACKAGE"
  [[ "$UX_CHROMIUM_BIN" =~ ^/[A-Za-z0-9._/+:-]+$ ]] || die "Некорректный абсолютный путь Chromium: $UX_CHROMIUM_BIN"
else
  [[ -z "$UX_CHROMIUM_PACKAGE$UX_CHROMIUM_BIN" ]] || die "Параметры Chromium разрешены только с --with-ux-acceptance."
fi
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
  git -C "$ROOT_DIR" rev-parse --verify HEAD >/dev/null 2>&1 || die "UX acceptance-профиль требует Git checkout с зафиксированным commit."
  UX_SOURCE_COMMIT="$(git -C "$ROOT_DIR" rev-parse HEAD)"
  [[ -z "$(git -C "$ROOT_DIR" status --porcelain --untracked-files=normal)" ]] || die "UX acceptance-профиль нельзя собирать из рабочего дерева с незакоммиченными изменениями."
fi
...
if [[ "$PREVIEW_PROFILE" == "with" && -z "$OS_PACKAGES_DIR" ]]; then die "Для --with-preview требуется --os-packages-dir с LibreOffice Writer/Calc."; fi
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" && -z "$OS_PACKAGES_DIR" ]]; then die "Для --with-ux-acceptance требуется --os-packages-dir с Chromium."; fi
if [[ -n "$OS_PACKAGES_DIR" ]]; then
  OS_PACKAGES_DIR="$(absolute_path "$OS_PACKAGES_DIR")"
  verify_os_package_set "$OS_PACKAGES_DIR" "$([[ "$PREVIEW_PROFILE" == "with" ]] && printf 1 || printf 0)"
  ...
  [[ "$TARGET_PROFILE" != "generic" && "$SOURCE_OS_FAMILY" == "$TARGET_PROFILE" ]] || die "--target-profile не совпадает с OS_FAMILY набора .deb"
  EXPECTED_DEB_ARCHITECTURE="$([[ "$NODE_ARCH" == "x64" ]] && printf amd64 || printf arm64)"
  [[ "$SOURCE_DEB_ARCHITECTURE" == "$EXPECTED_DEB_ARCHITECTURE" ]] || die "Архитектура набора .deb не совпадает с --target-arch"
  if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
    UX_CHROMIUM_PACKAGE_VERSION="$(awk -F '\t' -v package="$UX_CHROMIUM_PACKAGE" 'NR > 1 && $2 == package { print $3 }' "$OS_PACKAGES_DIR/packages.tsv")"
    [[ "$UX_CHROMIUM_PACKAGE_VERSION" =~ ^[A-Za-z0-9][A-Za-z0-9.+:~_-]*$ ]] || die "В наборе .deb отсутствует единственный пакет Chromium: $UX_CHROMIUM_PACKAGE"
  fi
fi

if ((WITHOUT_LLM == 0)); then
  [[ -n "$LLAMA_SERVER" && -n "$MODEL_FILE" ]] || die "Укажите одновременно --llama-server и --model либо явно используйте --without-llm."
  ...
  [[ "$MODEL_BASENAME" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || die "Имя файла модели содержит запрещённые символы: $MODEL_BASENAME"
fi
...
[[ "$ACTUAL_NODE_VERSION" == "v$NODE_VERSION" ]] || warn "Requested Node.js v$NODE_VERSION, runtime reports $ACTUAL_NODE_VERSION"
...
GIT_COMMIT="unknown"
if git -C "$ROOT_DIR" rev-parse --verify HEAD >/dev/null 2>&1; then GIT_COMMIT="$(git -C "$ROOT_DIR" rev-parse HEAD)"; fi
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
  [[ "$(git -C "$ROOT_DIR" rev-parse HEAD)" == "$UX_SOURCE_COMMIT" ]] || die "HEAD изменился во время сборки; UX acceptance-комплект не создан."
  [[ -z "$(git -C "$ROOT_DIR" status --porcelain --untracked-files=normal)" ]] || die "Сборка или lifecycle-скрипт изменили Git checkout; UX acceptance-комплект не создан."
  [[ "$GIT_COMMIT" =~ ^([a-f0-9]{40}|[a-f0-9]{64})$ ]] || die "UX acceptance-профиль можно собрать только из зафиксированного Git commit."
fi
...
mkdir -p "$BUNDLE_DIR/payload/app/apps/api" "$BUNDLE_DIR/payload/app/apps/worker" "$BUNDLE_DIR/payload/app/packages/config" "$BUNDLE_DIR/payload/app/packages/contracts" "$BUNDLE_DIR/payload/app/packages/storage" "$BUNDLE_DIR/payload/app/packages/document-intake" "$BUNDLE_DIR/payload/app/packages/template-compiler" "$BUNDLE_DIR/payload/app/scripts/runtime" "$BUNDLE_DIR/payload/app/scripts/ci" "$BUNDLE_DIR/payload/app/examples" "$BUNDLE_DIR/payload/app/docs" "$BUNDLE_DIR/payload/runtime/node" "$BUNDLE_DIR/payload/runtime/llama" "$BUNDLE_DIR/payload/models" "$BUNDLE_DIR/payload/deploy/systemd" "$BUNDLE_DIR/payload/config" "$BUNDLE_DIR/payload/os-packages"
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
  mkdir -p "$BUNDLE_DIR/payload/acceptance/ux/tests/e2e" "$BUNDLE_DIR/payload/acceptance/ux/node_modules/@playwright" "$BUNDLE_DIR/payload/acceptance/ux/node_modules/@axe-core"
fi

cp "$ROOT_DIR/package.json" "$ROOT_DIR/package-lock.json" "$ROOT_DIR/VERSION" "$ROOT_DIR/README.md" "$BUNDLE_DIR/payload/app/"
...
cp "$ROOT_DIR/scripts/ci/release-gate.mjs" "$ROOT_DIR/scripts/ci/release-gate-crash-worker.mjs" "$ROOT_DIR/scripts/ci/libreoffice-release-gate.mjs" "$BUNDLE_DIR/payload/app/scripts/ci/"
...
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
  UX_E2E_FILES=("README.md" "accessibility-audit.spec.mjs" "bulk-import.spec.mjs" "employee-card.spec.mjs" "generic-entities.spec.mjs" "fixtures/docomator-api.mjs" "fixtures/test.mjs" "navigation-and-accessibility.spec.mjs" "operation-center.spec.mjs" "pages/docomator-page.mjs" "playwright.config.mjs" "reporters/axe-json-reporter.mjs" "template-and-generation.spec.mjs" "visual-artifacts.spec.mjs")
  for relative in "${UX_E2E_FILES[@]}"; do
    source_file="$ROOT_DIR/tests/e2e/$relative"
    [[ -f "$source_file" && ! -L "$source_file" ]] || die "Файл UX acceptance-набора отсутствует или является ссылкой: $relative"
    destination="$BUNDLE_DIR/payload/acceptance/ux/tests/e2e/$relative"
    mkdir -p "$(dirname "$destination")"
    cp "$source_file" "$destination"
  done
  cp -a "$ROOT_DIR/node_modules/@playwright/test" "$BUNDLE_DIR/payload/acceptance/ux/node_modules/@playwright/"
  cp -a "$ROOT_DIR/node_modules/@axe-core/playwright" "$BUNDLE_DIR/payload/acceptance/ux/node_modules/@axe-core/"
  for package in playwright playwright-core axe-core; do cp -a "$ROOT_DIR/node_modules/$package" "$BUNDLE_DIR/payload/acceptance/ux/node_modules/"; done
fi

EXAMPLE_FILES=("README.md" "manifest.sha256" "data/auditoriums.csv" "data/employees.csv" "data/scientific-articles.csv" "fixtures/header-field.docx" "fixtures/rejected/macro-part.docx" "fixtures/scalar-fields.xlsx" "templates/personal-card.docx" "templates/team-register.docx" "templates/team-register.xlsx" "expected/personal-card-filled.docx" "expected/team-register-filled.docx" "expected/team-register-filled.xlsx")
for relative in "${EXAMPLE_FILES[@]}"; do
  source_file="$ROOT_DIR/examples/$relative"
  [[ -f "$source_file" && ! -L "$source_file" ]] || die "Учебный пример отсутствует или является ссылкой: $relative"
...
replace_env_value "$BUNDLE_DIR/payload/config/docomator.env.example" DOCOMATOR_PREVIEW_ENABLED "$([[ "$PREVIEW_PROFILE" == "with" ]] && printf true || printf false)"
...
if [[ -n "$OS_PACKAGES_DIR" ]]; then
  while IFS= read -r -d '' package_file; do cp "$package_file" "$BUNDLE_DIR/payload/os-packages/"; done < <(find "$OS_PACKAGES_DIR" -maxdepth 1 -type f \( -name '*.deb' -o -name 'manifest.sha256' -o -name 'packages.tsv' -o -name 'requested-packages.txt' -o -name 'source-os.env' \) -print0 | LC_ALL=C sort -z)
fi
...
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
  [[ "$(git -C "$ROOT_DIR" rev-parse HEAD)" == "$UX_SOURCE_COMMIT" ]] || die "HEAD изменился при копировании payload; UX acceptance-комплект не создан."
  [[ -z "$(git -C "$ROOT_DIR" status --porcelain --untracked-files=normal)" ]] || die "Git checkout изменился при копировании payload; UX acceptance-комплект не создан."
fi

MODEL_NAME=""; MODEL_SHA256=""; LLAMA_SHA256=""
if ((WITHOUT_LLM == 0)); then MODEL_NAME="$MODEL_BASENAME"; MODEL_SHA256="$(sha256_of "$MODEL_FILE")"; LLAMA_SHA256="$(sha256_of "$LLAMA_SERVER")"; fi
...
OS_PACKAGES_INCLUDED=false; OS_PACKAGES_MANIFEST_SHA256=""; OS_PACKAGES_INVENTORY_SHA256=""; OS_PACKAGE_SOURCE_JSON="null"
if [[ -n "$OS_PACKAGES_DIR" ]]; then
...
UX_ACCEPTANCE_INCLUDED=false; UX_CHROMIUM_PACKAGE_METADATA=""; UX_CHROMIUM_PACKAGE_VERSION_METADATA=""; UX_CHROMIUM_PATH_METADATA=""; UX_PLAYWRIGHT_VERSION=""; UX_AXE_PLAYWRIGHT_VERSION=""
if [[ "$UX_ACCEPTANCE_PROFILE" == "with" ]]; then
  ...
  UX_PLAYWRIGHT_VERSION="$("$BUNDLED_NODE" -e 'const fs=require("node:fs"); console.log(JSON.parse(fs.readFileSync(process.argv[1], "utf8")).version)' "$ROOT_DIR/node_modules/playwright/package.json")"
  UX_AXE_PLAYWRIGHT_VERSION="$("$BUNDLED_NODE" -e 'const fs=require("node:fs"); console.log(JSON.parse(fs.readFileSync(process.argv[1], "utf8")).version)' "$ROOT_DIR/node_modules/@axe-core/playwright/package.json")"
  [[ "$UX_PLAYWRIGHT_VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ && "$UX_AXE_PLAYWRIGHT_VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || die "Не удалось определить закреплённые версии Playwright/axe."
fi
