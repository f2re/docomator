...
async function systemInformation(config) {
  ...
  const [uname, libc, node, libreOffice] = await Promise.all([
    ...
    Promise.resolve({ ok: true, stdout: process.version, stderr: "", error: null }),
...
async function systemdChecks(config) {
  ...
  return [
    check({ id: "systemd_api", title: "Служба API", state: api.ok && api.stdout === "active" ? "ok" : "error", required: true, summary: api.ok && api.stdout === "active" ? "Служба API активна" : "Служба API не активна", detail: api.stdout || api.stderr || api.error, remediation: "Выполните systemctl status docomator-api.service и устраните ошибку запуска." }),
    check({ id: "systemd_worker", title: "Фоновый обработчик", state: worker.ok && worker.stdout === "active" ? "ok" : "error", required: true, summary: worker.ok && worker.stdout === "active" ? "Служба worker активна" : "Служба worker не активна", detail: worker.stdout || worker.stderr || worker.error, remediation: "Выполните systemctl status docomator-worker.service и устраните ошибку запуска." }),
    check({ id: "backup_timer", title: "Таймер резервных копий", state: !backupEnabled ? "disabled" : timerEnabled.ok && timerEnabled.stdout === "enabled" && timerActive.ok && timerActive.stdout === "active" ? "ok" : "error", required: backupEnabled, summary: !backupEnabled ? "Автоматические копии отключены" : timerEnabled.ok && timerActive.ok ? "Таймер включён и активен" : "Таймер не включён или не активен", detail: timerLine.stdout || timerEnabled.stdout || timerActive.stdout || timerLine.error, remediation: backupEnabled ? "Выполните systemctl enable --now docomator-backup.timer." : "Включите DOCOMATOR_BACKUP_ENABLED=true перед рабочей эксплуатацией." })
  ];
}
...
async function latestVerifiedBackup(dataDirectory) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const manifestSha256 = createHash("sha256").update(await fs.readFile(path.join(candidate.directory, "manifest.sha256"))).digest("hex");
      return { ok: true, backup: { directory: candidate.directory, createdAt: manifest.createdAt, releaseVersion: manifest.releaseVersion, manifestSha256, databaseBytes: manifest.database?.sizeBytes ?? null, objectCount: manifest.objects?.count ?? null, configIncluded: manifest.config !== null }, error: null };
    } catch {}
  }
  return { ok: false, backup: null, error: "Проверенная резервная копия не найдена" };
}
...
async function backupChecks(config, options, dataDirectory) {
  ...
  if (options.runBackup) {
    ...
    controlCommand = await runCommand("systemctl", ["start", "docomator-backup.service"], 6 * 60 * 60 * 1000);
  }
  if (!backupEnabled && !options.runBackup) {
    checks.push(check({ id: "backup_verified", title: "Восстановимая резервная копия", state: "disabled", required: false, summary: "Автоматическое резервирование отключено", remediation: "Включите резервирование перед рабочей эксплуатацией." }));
    return checks;
  }
  const latest = await latestVerifiedBackup(dataDirectory);
  if (options.runBackup) {
    const controlEvidence = evaluateControlBackup({ commandOk: controlCommand?.ok === true, commandDetail: controlCommand?.stdout || controlCommand?.stderr || controlCommand?.error || null, startedAt: controlStartedAt, backup: latest.ok ? latest.backup : null, expectedReleaseVersion: config.DOCOMATOR_VERSION || null });
    checks.push(check({ id: "backup_run", title: "Контрольный запуск резервирования", state: controlEvidence.ok ? "ok" : "error", required: true, summary: controlEvidence.summary, detail: controlEvidence.detail, remediation: controlEvidence.remediation, data: controlEvidence.data }));
  }
  const ageHours = latest.ok ? (Date.now() - Date.parse(latest.backup.createdAt)) / 3_600_000 : Number.POSITIVE_INFINITY;
  const fresh = latest.ok && Number.isFinite(ageHours) && ageHours <= 48;
  checks.push(check({ id: "backup_verified", title: "Восстановимая резервная копия", state: !latest.ok ? "error" : fresh ? "ok" : "warning", required: backupEnabled, summary: !latest.ok ? "Проверенная копия не найдена" : fresh ? "Недавняя копия полностью проверена" : "Проверенная копия старше 48 часов", detail: latest.ok ? `${latest.backup.createdAt}; ${latest.backup.directory}` : latest.error, remediation: latest.ok && fresh ? "На отдельном стенде выполните пробное восстановление этой копии." : "Запустите systemctl start docomator-backup.service и повторите проверку.", data: latest.backup ?? {} }));
  return checks;
}
...
function importServerChecks(serverReport, options) {
  ...
  return (serverReport.checks ?? []).map((item) => {
    const required = item.required || (options.requireNetwork && item.id === "network_folder") || (options.requireSmtp && item.id === "smtp");
    let state = item.state;
    if (required && item.state === "disabled") state = "error";
    return check({ id: `server_${item.id}`, title: item.title, state, required, summary: item.summary, detail: item.detail, remediation: item.remediation, data: item.data });
  });
}
...
function markdownReport(report) {
  ...
  for (const item of report.checks) {
    ...
    if (item.remediation) lines.push(`|  |  |  | Действие: ${String(item.remediation).replaceAll("|", "\\|")} |`);
  }
  lines.push("", "## Сводка", "", `- Успешно: ${report.summary.ok}`, `- Предупреждений: ${report.summary.warning}`, `- Ошибок: ${report.summary.error}`, `- Отключено: ${report.summary.disabled}`, `- Блокирующих ошибок: ${report.summary.requiredErrors}`, "");
  return `${lines.join("\n")}\n`;
}

const options = parseArguments(process.argv.slice(2));
const configFile = path.resolve(options.configFile);
const config = (await exists(configFile)) ? parseEnv(await fs.readFile(configFile, "utf8")) : {};
const dataDirectory = path.resolve(config.DOCOMATOR_DATA_DIR || process.env.DOCOMATOR_DATA_DIR || "/var/lib/docomator");
...
const requiredErrors = checks.filter((item) => item.required && item.state === "error").length;
const summary = { ok: checks.filter((item) => item.state === "ok").length, warning: checks.filter((item) => item.state === "warning").length, error: checks.filter((item) => item.state === "error").length, disabled: checks.filter((item) => item.state === "disabled").length, requiredErrors };
const status = requiredErrors > 0 ? "failed" : summary.warning > 0 || summary.error > 0 ? "attention" : "passed";
const report = {
...
const result = { status, jsonReport: jsonPath, markdownReport: markdownPath, summary };
if (options.jsonOnly) process.stdout.write(`${JSON.stringify(result)}\n`);
else process.stdout.write(`${status === "passed" ? "✅" : status === "attention" ? "⚠️" : "⛔"} ${status === "passed" ? "Пилотный контур готов." : status === "attention" ? "Пилотный контур работает с предупреждениями." : "Пилотный запуск заблокирован."}\nJSON: ${jsonPath}\nОтчёт: ${markdownPath}\n`);
