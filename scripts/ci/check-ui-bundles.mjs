...
try {
  for (const [bundleName, fileNames] of Object.entries(bundles)) {
    ...
    await fs.writeFile(
      bundlePath,
      Buffer.concat(parts.flatMap((part, index) => index === 0 ? [part] : [Buffer.from("\n\n"), part]))
    );
...
if (process.exitCode === undefined) process.stdout.write("Пользовательские UI-бандлы прошли синтаксическую проверку.\n");
