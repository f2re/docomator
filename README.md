...
**Текущее состояние:** универсальные данные, пространства, аудитории, русскоязычный интерфейс, безопасная проверка, неизменяемое хранение исходников и ограниченное структурное представление DOCX/XLSX работают. Следующий вертикальный шаг — сохранить выбранный абзац или ячейку как поле черновика и скомпилировать техническую привязку.  
**Среда:** Node.js 24 LTS, TypeScript, SQLite, `llama.cpp`, Debian/Astra Linux, работа без доступа в Интернет.

> [!IMPORTANT]
> Docomator пока не является завершённой системой формирования документов. Уже работают данные, пространства, группы, снимки состава, очередь, аудит, резервное копирование, безопасная проверка файлов, подтверждаемое хранение исходников и структурный анализ. Физическое заполнение DOCX/XLSX ещё не выдается за готовую функцию.
...
- увидеть абзацы DOCX, текстовые фрагменты, листы и ячейки XLSX;
- выбрать изменяемое место вручную либо получить предложение локальной модели;
- связать поле документа с произвольными сведениями о людях, организациях и других объектах;
...
| Безопасный приём DOCX/XLSX | 🟡 | проверка пакета, карантин, абзацы DOCX, листы и ячейки XLSX; сохранение поля ещё впереди |
| Веб-интерфейс | 🟡 | автономная оболочка, база знаний, пространства, проверка файлов, структура, помощь и понятные состояния |
| Автономная поставка | 🟡 | комплект поставки, SHA-256, установка, обновление, откат и помощник первого запуска |
| Компилятор шаблонов | ⬜ | следующий крупный этап после сохранения поля |
...
> [!NOTE]
> Выбор аудитории и точное число будущих документов уже реализованы. Подстановка списка участников в повторяющиеся строки DOCX/XLSX будет добавлена после базового скалярного компилятора.

Технический контракт: [пространства и аудитории](docs/SPACES_AND_AUDIENCES.md).

## 🛡️ Проверка, хранение и структура DOCX/XLSX

Пользователь проходит последовательные понятные шаги:

```text
1. Выбрать файл
2. Проверить безопасность
3. Подтвердить сохранение исходника
...
После отдельного подтверждения система вычисляет SHA-256 и сохраняет неизменяемую копию внутри выбранного пространства. Одинаковый файл не дублируется.

Структурный анализ возвращает только безопасные данные:

- DOCX: основной текст, колонтитулы, сноски, абзацы, текстовые фрагменты и положение внутри таблицы;
- XLSX: листы, адреса ячеек, значения, формулы и тип значения;
- устойчивый идентификатор из SHA-256 исходника и структурной координаты;
- контрольную сумму структуры;
- признак сокращённого показа крупного документа.

Исходный XML в браузер не передаётся. Объявления `DOCTYPE` и `ENTITY` блокируются до структурного разбора.
...
1. сохранить выбранный абзац или ячейку как поле черновика;
2. зафиксировать название, ключ, тип, обязательность и контрольную сумму структуры;
3. проверить координату на сервере;
4. создать для DOCX элемент управления содержимым, для XLSX — именованную привязку;
5. вставить пробное значение и считать его обратно;
6. запретить активацию версии при ошибке;
7. затем добавить повторяющийся блок для списка `audience.members`.
...
- 🧭 понятный список абзацев и ячеек с устойчивыми координатами;
...
→ структурный анализ → выбор будущего поля
```
