...
**Текущее состояние:** работают универсальные данные, пространства и аудитории, безопасный приём DOCX/XLSX, многополевая компиляция и проверка, сохраняемый PDF-предпросмотр, явная активация и единый каталог одно- и многополевых версий. Следующий этап — повторяющиеся блоки для общего документа.
**Среда:** Node.js 24 LTS, TypeScript, SQLite, `llama.cpp`, Debian/Astra Linux, работа без доступа в Интернет.

> [!IMPORTANT]
> Docomator пока не является завершённой системой формирования документов. Обычный шаблон с несколькими скалярными полями уже компилируется, заполняется, проверяется, просматривается в PDF и активируется. Повторяющиеся таблицы `audience.members`, рабочие документные задания, расписания и доставка ещё не завершены.
...
| Веб-интерфейс | 🟡 | автономная оболочка, база знаний, пространства, многополевая студия шаблонов, помощь и явные состояния |
| Автономная поставка | 🟡 | комплект, SHA-256, установка, обновление, откат и помощник первого запуска |
| Компилятор и активные версии | ✅ | несколько полей компилируются, проверяются, просматриваются и активируются одной версией |
| Формирование DOCX/XLSX | 🟡 | скалярные технические и активные версии работают; повторяющиеся блоки и задания впереди |
...
> [!NOTE]
> Состав аудитории и точное число будущих документов уже рассчитываются. Фактическая повторяющаяся таблица для `audience.members` будет добавлена следующим вертикальным приращением.
...
## 🧪 Пробное заполнение и обратная проверка
...
> [!IMPORTANT]
> Одно- и многополевые проверенные версии проходят один процесс выпуска. Прежние одно-полевые версии сохраняются для совместимости, а каталог имеет один текущий указатель на черновик.

## 👁️ Предварительный просмотр и активация

Проверенная одно- или многополевая версия проходит отдельный сохраняемый процесс:

```text
проверенная версия → единый кандидат на выпуск
→ очередь → LibreOffice с отдельным профилем
...
Активная версия неизменяема. Манифест многополевой версии включает все поля, исходные и технические привязки. Отдельный указатель определяет текущую версию, поэтому прежние версии и исторические задания не изменяются.

Подробности: [предварительный просмотр и активация](docs/TEMPLATE_ACTIVATION.md) и [многополевая компиляция](docs/MULTI_FIELD_COMPILER.md).

## 🧱 Следующий вертикальный шаг

Продолжение выполняется без локальной модели:

1. добавить отдельный тип привязки повторяющегося блока;
2. реализовать одну повторяемую строку таблицы DOCX для `audience.members`;
3. ограничить число строк и проверить структуру результата;
4. затем добавить повторяемую строку XLSX;
5. создать один сводный либо несколько индивидуальных документов;
6. связать активный каталог с ручным документным заданием.
...
- 🧩 полный набор полей, история и многополевая проверка;
- 👁️ выбор проверенной версии, PDF-просмотр и явная активация;
...
flowchart TB
    ...
    DRAFT --> RELEASE[Проверенная и активная версия]
...
curl http://127.0.0.1:8080/api/v1/spaces/default/active-templates
```
...
→ структурный анализ → сохранение полей
→ многополевая проверка → PDF → активация
```
