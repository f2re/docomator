...
import { createSqlitePasswordCredentialStore, installPasswordGate, loadPasswordGateConfig } from "./password-gate.js";
...
installPasswordGate(app, loadPasswordGateConfig(), createSqlitePasswordCredentialStore(store));
...
async function shutdown(signal: NodeJS.Signals): Promise<void> {
  if (closing) return;
  closing = true; app.log.info({ signal }, "shutdown requested");
  const timer = setTimeout(() => { app.log.error("graceful shutdown timed out"); process.exit(1); }, 10_000); timer.unref();
  try { await app.close(); store.close(); process.exit(0); }
...
try {
  ...
  if (typeof process.send === "function" && process.connected && address !== null && typeof address === "object") process.send({ type: "listening", host: config.host, port: address.port });
} catch (error) { app.log.fatal({ error }, "api failed to start"); store.close(); process.exit(1); }
