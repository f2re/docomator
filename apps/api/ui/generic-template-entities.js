{
  ...
  function genericTemplateSpaceId() {
    return String(globalThis.docomatorTemplateWizard?.spaceId?.() || globalThis.docomatorCurrentSpaceId || "");
  }
  ...
  function genericTemplateType() {
    return genericTemplateEntityTypes.find((type) => type.key === genericTemplateEntityTypeKey) || null;
  }
  ...
  function genericTemplatePublishType() {
    ...
    localStorage.setItem(genericTemplateTypeStorageKey(), genericTemplateEntityTypeKey);
    const system = structureSystemPropertyDefinitions[0];
    if (system) {
      system.label = genericTemplateIsPerson() ? "ФИО участника" : "Название объекта";
  ...
  loadStructurePropertyDefinitions = async function loadGenericStructureDefinitions() {
    ...
    structurePropertyDefinitions = Array.isArray(properties.data) ? properties.data : [];
    ...
    genericTemplateEntityTypeKey =
      candidates.find((key) => genericTemplateEntityTypes.some((type) => type.key === key)) || "person";
    genericTemplatePublishType();
  };

  structurePropertyOptions = function genericStructurePropertyOptions(selectedGroup = "common") {
    ...
    const applicable = structurePropertyDefinitions
      .filter((definition) => {
        ...
        return appliesTo.length === 0 || appliesTo.includes(genericTemplateEntityTypeKey);
      })
      .filter((definition) =>
        !person ||
  ...
  structureFetchJson = async function genericTemplateFetch(url, options = {}) {
    if (
      ...
      payload.sensitivity = genericTemplateIsPerson() ? payload.sensitivity || "personal" : "internal";
      payload.validation = genericTemplateIsPerson()
        ? { ...(payload.validation || {}), uiGroup: payload.validation?.uiGroup || "common" }
        : { ...(payload.validation || {}) };
  ...
  structureSelectedPersonName = function genericSelectedPersonName(form = document) {
    return genericTemplateIsPerson() ? genericTemplateBasePersonName(form) : null;
  };
  ...
  renderStructureFormatterFields = function renderGenericStructureFormatter() {
    ...
    const presentation = document.querySelector("#documentFieldTextPresentation");
    ...
    if (hint) hint.textContent = "Для произвольных объектов текст подставляется без преобразования ФИО.";
    document.querySelector("#documentFieldNameOptions")?.setAttribute("hidden", "");
  ...
  structureFieldBlockReason = function genericStructureFieldBlockReason(form) {
    return genericTemplateBaseBlockReason(form)
  ...
  structureRepeatRowControl = function genericStructureRepeatControl(element) {
    return genericTemplateBaseRepeatControl(element)
  ...
  function genericTemplateAdaptSelection() {
    ...
    if (!field) {
      ...
      const groupField = form.querySelector("#documentFieldGroup")?.closest("label");
      grid.insertBefore(field, groupField || grid.firstChild);
      field.querySelector("#documentEntityType")?.addEventListener("change", (event) => {
        genericTemplateEntityTypeKey = event.target.value;
        genericTemplatePublishType();
        refreshStructurePropertySelector();
        genericTemplateAdaptSelection();
      });
      globalThis.docomatorSearchableSelect?.enhance(field.querySelector("#documentEntityType"));
    } else {
      const select = field.querySelector("#documentEntityType");
      select.innerHTML = genericTemplateSelectOptions(genericTemplateEntityTypeKey);
    ...
    const confirmation = form.querySelector("#documentNewPropertyFields .structure-required-field span");
    if (confirmation && !genericTemplateIsPerson()) {
      confirmation.innerHTML = `<strong>Добавить поле объектам типа «${structureEscape(genericTemplateType()?.label || genericTemplateEntityTypeKey)}»</strong><small>Поле станет доступно другим шаблонам этого типа.</small>`;
    }
    const selectorLabel = form.querySelector("#documentFieldProperty")?.closest("label")?.querySelector("span");
    if (selectorLabel) selectorLabel.textContent = "Какое поле объекта поставить сюда?";
    const selectorHint = form.querySelector("#documentFieldProperty")?.closest("label")?.querySelector("small");
    if (selectorHint) {
  ...
  function genericTemplateAdaptStaticText() {
    ...
    if (intro) intro.textContent = "Система покажет текст и ячейки документа. Нажмите на нужное место, выберите тип объектов и его поле.";
  }
