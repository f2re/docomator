  ...
  holder.innerHTML = `
      ...
      <form id="scheduleForm" class="schedule-form" novalidate>
        <div class="schedule-grid">
          <label class="generation-field">
            <span>Название расписания</span>
            <input id="scheduleName" type="text" maxlength="300" placeholder="Например: ежемесячная ведомость" required />
          </label>
          <label class="generation-field">
            <span>Устойчивый ключ</span>
            <input id="scheduleKey" type="text" maxlength="160" placeholder="monthly-report" required />
            <small>Ключ используется для защиты периодов от повторного запуска.</small>
          </label>
          <label class="generation-field">
            <span>Активный шаблон</span>
            <select id="scheduleTemplate">${scheduleTemplates
              .map(
                (template) =>
                  `<option value="${generationEscape(template.id)}">${generationEscape(template.title)} · ${String(template.format).toUpperCase()}</option>`
              )
              .join("")}</select>
          </label>
          <label class="generation-field">
            <span>Сохранённая группа</span>
            <select id="scheduleGroup">${activeGroups
              .map(
                (group) =>
          ...
          <label class="generation-field">
            ...
            <select id="scheduleTargetMode">
              <option value="one_per_member">Документ на каждого участника</option>
              <option value="aggregate">Один сводный документ</option>
            </select>
          </label>
          <label class="generation-field">
          ...
          <label class="generation-field">
            ...
            <input id="scheduleStartDate" type="date" value="${scheduleToday()}" required />
          </label>
          <label class="generation-field">
            <span>Локальное время</span>
            <input id="scheduleLocalTime" type="time" value="${scheduleTimeSoon()}" required />
          </label>
          <label class="generation-field">
            <span>Часовой пояс IANA</span>
            <input id="scheduleTimezone" type="text" maxlength="100" value="${generationEscape(scheduleTimezone())}" required />
            <small>Например: Europe/Moscow или Asia/Yekaterinburg.</small>
          </label>
          <label class="generation-field" id="scheduleMonthlyField" hidden>
            <span>День месяца</span>
            <input id="scheduleDayOfMonth" type="number" min="1" max="28" value="1" />
            <small>Первая версия ограничена днями 1–28.</small>
          </label>
          <label class="generation-field">
            <span>Автоматическая доставка</span>
            <select id="scheduleDeliveryChannel">
              <option value="none">Не доставлять автоматически</option>
              <option value="email" ${scheduleRecipients.length === 0 ? "disabled" : ""}>Отправить по электронной почте</option>
            </select>
          </label>
          <label class="generation-field document-email-wide">
            <span>Описание</span>
            <input id="scheduleDescription" type="text" maxlength="2000" placeholder="Необязательно" />
          </label>
          <section id="scheduleEmailFields" class="schedule-email-fields document-email-wide" hidden>
            <label class="generation-field">
              <span>Сохранённый получатель</span>
              <select id="scheduleRecipient">${scheduleRecipients
            ...
            <label class="generation-field">
              ...
              <input id="scheduleEmailSubject" type="text" maxlength="300" value="Документы: {template} · {period}" />
            </label>
            <label class="generation-field document-email-wide">
              <span>Текст письма</span>
              <textarea id="scheduleEmailText" rows="4" maxlength="20000">Автоматический выпуск «{schedule}» за период {period}.\nШаблон: {template}.\nГруппа: {group}.\n\nДокументы находятся во вложении.</textarea>
              <small>Доступны подстановки: {schedule}, {period}, {template}, {group}.</small>
            </label>
          </section>
        </div>
        <div class="generation-actions">
          <button id="scheduleSubmit" class="primary-button" type="submit">Создать расписание</button>
          <p id="scheduleMessage">Перед запуском система создаёт новый снимок группы и проверяет обязательные данные.</p>
        </div>
      </form>` : `
    ...
    <section class="schedule-list-section">
      <div class="generation-history-heading">
        <div><p class="eyebrow">Действующие правила</p><h3>Расписания пространства</h3></div>
        <button class="secondary-button" id="scheduleRefresh" type="button">Обновить</button>
      </div>
      <div id="scheduleList" class="schedule-list">${renderScheduleListHtml()}</div>
    </section>`;
  holder
    .querySelector("#scheduleRecurrence")
    ?.addEventListener("change", updateScheduleConditionalFields);
  holder
    .querySelector("#scheduleDeliveryChannel")
    ?.addEventListener("change", updateScheduleConditionalFields);
  holder
    .querySelector("#scheduleForm")
    ?.addEventListener("submit", submitSchedule);
  holder
    .querySelector("#scheduleRefresh")
    ?.addEventListener("click", loadScheduleWorkspace);
...
function bindScheduleListActions() {
  document.querySelectorAll("[data-schedule-run]").forEach((button) =>
    button.addEventListener("click", () => runScheduleNow(button.dataset.scheduleRun || ""))
  );
  document.querySelectorAll("[data-schedule-runs]").forEach((button) =>
    button.addEventListener("click", () => loadScheduleRuns(button.dataset.scheduleRuns || ""))
  );
...
async function submitSchedule(event) {
  ...
  try {
    const body = await generationFetchJson(
      `/api/v1/spaces/${encodeURIComponent(currentGenerationSpaceId())}/document-schedules`,
      {
        method: "POST",
...
async function setScheduleStatus(scheduleId, status) {
  ...
  try {
    await generationFetchJson(
      `/api/v1/spaces/${encodeURIComponent(currentGenerationSpaceId())}/document-schedules/${encodeURIComponent(scheduleId)}/status`,
      {
        method: "PUT",
        headers: { "content-type": "application/json" },
    ...
    if (list) {
      list.insertAdjacentHTML(
        "afterbegin",
        `<div class="generation-state is-error"><span aria-hidden="true">⛔</span><div><strong>Состояние не изменено</strong><p>${generationEscape(error?.message || "Повторите действие.")}</p></div></div>`
      );
    }
  } finally {
    scheduleBusy = false;
...
async function runScheduleNow(scheduleId) {
  ...
  try {
    const body = await generationFetchJson(
      `/api/v1/spaces/${encodeURIComponent(currentGenerationSpaceId())}/document-schedules/${encodeURIComponent(scheduleId)}/run-now`,
    ...
    if (holder) {
      holder.insertAdjacentHTML(
        "afterbegin",
        `<div class="generation-state is-pending"><span aria-hidden="true">⏳</span><div><strong>Ручной запуск создан</strong><p>Период: ${generationEscape(body.data.periodKey)}. Worker подхватит его без изменения календаря.</p></div></div>`
      );
    }
  } catch (error) {
    const holder = document.querySelector(`#scheduleRuns_${CSS.escape(scheduleId)}`);
...
async function loadScheduleRuns(scheduleId, forceOpen = false) {
  ...
  try {
    const body = await generationFetchJson(
      `/api/v1/spaces/${encodeURIComponent(currentGenerationSpaceId())}/document-schedules/${encodeURIComponent(scheduleId)}/runs?limit=50`
    );
    const runs = Array.isArray(body.data) ? body.data : [];
    holder.innerHTML = runs.length === 0
      ? `<div class="generation-history-empty">Запусков пока нет.</div>`
      : runs
          .map(
            (run) => `
              <article class="schedule-run is-${generationEscape(run.state)}">
                <div>
                  <span class="generation-state-code">${generationEscape(scheduleStateLabel(run.state))}</span>
                  <strong>${generationEscape(run.periodKey)}</strong>
                  <span>Назначен: ${generationEscape(new Date(run.dueAt).toLocaleString("ru-RU"))}</span>
                  ${scheduleResultMessage(run) ? `<span>${generationEscape(scheduleResultMessage(run))}</span>` : ""}
                </div>
                <div class="generation-history-actions">
                  ${run.documentJobId ? `<button class="secondary-button" type="button" data-open-schedule-job="${generationEscape(run.documentJobId)}">Открыть выпуск</button>` : ""}
                </div>
              </article>`
          )
          .join("");
    holder.querySelectorAll("[data-open-schedule-job]").forEach((button) =>
      button.addEventListener("click", () => {
...
function bindScheduleSpaceSelect() {
  ...
  if (!candidate || candidate === scheduleSpaceSelect) return;
...
if (scheduleView) {
  createSchedulePanel();
  bindScheduleSpaceSelect();
  new MutationObserver(() => {
    bindScheduleSpaceSelect();
    if (scheduleReloadTimer !== null) clearTimeout(scheduleReloadTimer);
    scheduleReloadTimer = setTimeout(() => {
      scheduleReloadTimer = null;
      if (!scheduleBusy) void loadScheduleWorkspace();
    }, 750);
  }).observe(scheduleView, { childList: true, subtree: true });
}
