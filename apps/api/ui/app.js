...
const help = {
  ...
  spaces: [
    ["Пространство и группа — это одно?", "Нет. Пространство помогает не смешивать данные разных подразделений или проектов, но не ограничивает доступ. Группа — сохранённый набор людей внутри выбранного пространства."],
...
templates: [
  ...
  ["Сохраняется ли выбранный файл?", "После отдельного подтверждения принятый исходник сохраняется в локальном хранилище по SHA-256. Повторная проверка и структура строятся сервером из этой неизменяемой копии."],
  ...
  automations: [
    ["Как расписание выбирает объекты?", "Расписание использует сохранённую группу выбранного типа и фиксирует её состав перед каждым календарным выпуском."],
    ["Что произойдёт при нехватке данных?", "Предварительная проверка остановит неполный сводный выпуск. Индивидуальный выпуск сохранит частичный результат и позволит повторить только проблемные строки после исправления."]
  ]
};
...
const dialogs = {
  ...
  space: {
    ...
    description: "Создайте отдельный раздел для подразделения, проекта или заказчика.",
    ...
    fields: [
      ...
      ["description", "Описание", "textarea", false, "Сотрудники и документы инженерной службы", "Объясните, какие данные относятся к этому пространству."]
    ],
  ...
  "space-entity": {
    ...
    description: "Запись будет относиться к текущему пространству.",
...
function renderSpaceSummary() {
  ...
  $("#spaceDescription").textContent = space.description || `Раздел «${space.name}» объединяет ${state.data.spaceEntities.length} участников и ${state.data.groups.length} групп.`;
}
...
function renderMembers() {
  ...
  if (state.data.spaceEntities.length === 0) {
    root.innerHTML = '<div class="empty-state compact-empty"><div><span class="empty-emoji" aria-hidden="true">👤</span><h3>В пространстве пока нет участников</h3><p>Добавьте первого человека. Его карточка будет относиться к этому пространству.</p><button class="primary-button" type="button" data-create="space-entity">Добавить участника</button></div></div>';
    updateSelectedCount();
    return;
  }
